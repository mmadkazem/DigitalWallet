...
{
    Task<string> Handle(LoginUserQueryRequest request, CancellationToken cancellationToken = default);
}
public sealed class LoginUserQueryHandler(AppDbContext context, ITokenFactoryService tokenFactory) : ILoginUserQueryHandler
{
    ...
    public async Task<string> Handle(LoginUserQueryRequest request, CancellationToken cancellationToken = default)
    {
